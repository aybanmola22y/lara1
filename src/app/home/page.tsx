...
import { Camera, Settings, RefreshCw, RotateCcw, Eye } from "lucide-react"
...
export default function HomePage() {
  ...
  const [showPreview, setShowPreview] = useState(false) // 👁️ Preview state
            ...
            <div className="absolute inset-0 rounded-xl border-4 border-dashed border-white/20 pointer-events-none">
              {countdown > 0 && (
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-8xl font-bold text-white animate-pulse drop-shadow-lg">{countdown}</div>
                </div>
              )}
            ...
            <div className={`grid ${getGridClass()} gap-2 aspect-square`}>
              {Array.from({ length: numPhotos }).map((_, index) => (
                <div key={index} className={`relative bg-gray-700 rounded-lg overflow-hidden ${index === 0 && numPhotos === 3 ? 'col-span-2' : ''}`}>
                  ...
                  <div className="absolute top-2 right-2 bg-black/50 text-white text-xs px-2 py-1 rounded">{index + 1}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {showPreview && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center px-4">
          <div className="bg-white rounded-lg p-4 max-w-4xl w-full relative">
            ...
            <h2 className="text-xl font-bold mb-4 text-center">Photo Collage Preview</h2>
            <div className={`grid ${getGridClass()} gap-2 aspect-square`}>
